...
test('the jobs section matches the high fidelity layout and retains live content', function () {
    ...
    expect($section)
        ->toContain("import { useHttp } from '@inertiajs/vue3'")
        ->toContain('VITE_PSBRS_VACANCIES_API_URL')
        ->toContain("'http://127.0.0.1:8000/api/v1/vacancies'")
        ->toContain('vacancies.value.slice(0, 6)')
        ->toContain('response.data.map(mapVacancy)')
        ->toContain('vacancy.position_name')
        ->toContain('vacancy.salary_grade')
        ->toContain('vacancy.monthly_salary')
        ->toContain('vacancy.appointment_type')
        ->toContain('vacancy.application_url')
        ...
        ->toContain('View Job')
        ->toContain('View All')
        ->toContain(':href="job.applicationUrl"')
        ->toContain(':href="vacanciesPageUrl"')
        ->toContain('target="_blank"')
        ->toContain('rel="noopener noreferrer"')
        ->toContain('Loading job opportunities')
        ->toContain('Job opportunities are temporarily unavailable.')
        ->toContain('Try Again')
        ...
        ->and($section)
        ->toContain("icon: 'hash'")
        ->toContain("icon: 'location'")
        ->toContain("v-if=\"metadata.icon === 'location'\"")
        ->toContain('>#</span')
        ->not->toContain('showIcon')
